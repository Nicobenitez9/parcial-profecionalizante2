document.addEventListener("DOMContentLoaded", function () {
          ...
          if (filteredProperties.length > 0) {
            filteredProperties.forEach(function (propiedad) {
              ...
              resultadoElement.innerHTML = `
                  <img class="property-image" src="${propiedad.image_url}" alt="${propiedad.title}">
                  <p>${propiedad.title} - Precio: $${propiedad.precio}</p>
                  <button class="buy-button" onclick="comprar('${propiedad.title}')">Comprar</button>
              `;
  ...
  function createPropertyCard(propiedad) {
    ...
    title.textContent = propiedad.title;
    ...
    priceLink.href = propiedad.link;
    priceLink.classList.add("btn", "btn-primary", "bg-transparent", "text-dark", "border-dark");
    priceLink.textContent = "Price";

    cardBody.appendChild(title);
    cardBody.appendChild(description);
    cardBody.appendChild(priceLink);
